...
async def on_ready():
    ...
    # Enviar mensagem para o terceiro canal
    channel_3 = bot.get_channel(CHANNEL_ID_3)
    if channel_3:
        try:
            await channel_3.send('Lembrar da Missao Canela e Acucar!!') 
            print(f'Mensagem enviada para o canal {channel_3.name}.')
        except Exception as e:
            print(f'Erro ao enviar mensagem para o canal {channel_3.name}: {e}')
    else:
