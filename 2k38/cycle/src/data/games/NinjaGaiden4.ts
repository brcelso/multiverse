export const ninja4 = {
  launchDate: '2025-10-20',
  Brazil: {
    'October 2025': [
      { edition: 'Deluxe Edition', price: 374.00, discount: 12, currency: 'R$', exchangeTax: 4.15 },
      { edition: 'Standard Edition', price: 299.00, discount: 25, currency: 'R$', exchangeTax: 4.27 },
    ],
    'June 2025': [
      { edition: 'Deluxe Edition', price: 424.00, currency: 'R$', exchangeTax: 4.71 },
      { edition: 'Standard Edition', price: 399.90, currency: 'R$', exchangeTax: 5.71 },
    ],
  },
  Europe: {
    'June 2025': [
      { edition: 'Deluxe Edition', price: 89.99, currency: '€' },
      { edition: 'Standard Edition', price: 69.99, currency: '€' },
    ],
  },
  UnitedKingdom: {
    'June 2025': [
      { edition: 'Deluxe Edition', price: 79.99, currency: '£' },
      { edition: 'Standard Edition', price: 59.99, currency: '£' },
    ],
  },
  UnitedStates: {
    'June 2025': [
      { edition: 'Deluxe Edition', price: 89.99, currency: '$' },
      { edition: 'Standard Edition', price: 69.99, currency: '$' },
    ],
  },
};